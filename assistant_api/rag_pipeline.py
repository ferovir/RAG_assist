...
from proxy_config import create_openai_client, get_proxy_info

from vector_store import (
    VectorStore,
    prompt_for_data_loading,
    prompt_for_collection_action,
    list_data_files,
)
...
class RAGPipeline:
    ...
    def __init__(self, 
                 ...
                 data_dir: str = "data",
                 load_documents: Optional[bool] = None,
        ...
        Args:
            ...
            data_dir: папка с документами для загрузки
            load_documents: загружать ли документы (None — спросить, по умолчанию no)
        ...
        self.proxy_info = get_proxy_info()
        if self.proxy_info["enabled"]:
            print(f"Прокси включён: {self.proxy_info['url']}")
        ...
        if not files:
            ...
            self._handle_data_loading(data_dir, load_documents)
    ...
    def _handle_data_loading(self, data_dir: str, load_documents: Optional[bool]):
        """Интерактивная или автоматическая загрузка документов из папки data."""
        should_load = load_documents
        if should_load is None:
            should_load = prompt_for_data_loading(data_dir)

        if not should_load:
            print("Загрузка документов пропущена")
            return
        ...
        if doc_count > 0:
            if load_documents is True:
                print("Коллекция не пуста — выполняется перезагрузка (режим evaluate)...")
                self.vector_store.clear_collection()
            else:
                action = prompt_for_collection_action(doc_count)
                if action == 'cancel':
                    print("Загрузка документов отменена")
                    return
                if action == 'clear':
                    self.vector_store.clear_collection()
                else:
        ...
        self.vector_store.load_documents_from_directory(data_dir, append=append)
