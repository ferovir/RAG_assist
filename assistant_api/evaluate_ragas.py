...
from proxy_config import configure_proxy_env, get_proxy_info

proxy = configure_proxy_env()
if proxy:
    print(f"[*] Прокси для OpenAI/RAGAS: {proxy}\n")
...
DEFAULT_QUESTIONS_FILE = Path(__file__).parent / "rag_test_questions.txt"


def load_evaluation_questions(file_path: Path = None) -> list:
    """
    Загрузка вопросов для оценки из файла.
    ...
    Returns:
        список вопросов
    """
    path = file_path or DEFAULT_QUESTIONS_FILE

    if not path.exists():
        raise FileNotFoundError(
            f"Файл с вопросами не найден: {path}\n"
            f"Создайте файл или укажите путь через переменную EVALUATION_QUESTIONS_FILE"
        )
...
def evaluate_rag_system():
    ...
    # Загрузка вопросов для оценки
    questions_file = Path(
        os.getenv("EVALUATION_QUESTIONS_FILE", DEFAULT_QUESTIONS_FILE)
    )
    try:
        evaluation_questions = load_evaluation_questions(questions_file)
        print(f"[*] Загружено {len(evaluation_questions)} вопросов из {questions_file.name}\n")
